async function set_username(){
    const username = form_post.querySelector(".box .username");
    const user = await fetch("get_session.php").then(onResponse);
...
function onWrite(event) {
    ...
    if(input.value.length > 0) {
        input.parentNode.querySelector(".error-element").classList.remove('error');
        formStatus = false;
    }
    else {
        input.parentNode.querySelector(".error-element").classList.add('error');
        formStatus = true;
    }
}
...
function onPost(event){
    ...
    const form = document.querySelector("form[name='post-form']");
    if(formStatus === false) {
        form.submit();
    }
}
...
async function onPostJson(json) {
    if (json.length == 0) {
        return;
    }
    ...
    //Richiedo al database i post a cui è stato messo like, in modo da caricarli con il cuore rosso
    //Utilizziamo la funzione asincrona in modo da attendere il risultato della fetch prima di continuare con il resto del codice
    const liked = await fetch("get_liked.php").then(onResponse);

    for(const post in json) {
        //Variabile per discernere se un post è tra i piaciuti
        let found = false;
        ...
        //Controllo se il post è presente nella lista dei post piaciuti
        ...
        like.src = found === true ? "utility/redlike.png" : "utility/like.png";
        like.dataset.like = found === true ? "like" : "none";
        ...
        form_post.parentNode.insertBefore(box, footer);
...
function calcRemain(event){
    const input = event.currentTarget;

    const remainingChars = input.parentNode.querySelector("#remain");

    const textLength = input.value.replace(/\s/g, '').length;
    const remaining = 1000 - textLength;
    remainingChars.textContent = remaining + ' caratteri rimanenti';
}
...
let formStatus = true;
const form_post = document.querySelector("form[name='post-form']");
form_post.querySelector("input").addEventListener('click', onPost);
form_post.querySelector("textarea").addEventListener('blur', onWrite);
form_post.querySelector("textarea").addEventListener('input', calcRemain);
